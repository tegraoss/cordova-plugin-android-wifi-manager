...
public class WifiManagerPlugin extends CordovaPlugin {
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
        ...
        wifiManager = (WifiManager) cordova.getActivity().getApplication().getApplicationContext()
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if (action.equals(ACTION_ADD_NETWORK))
            addNetwork(args, callbackContext);
        else if (action.equals(ACTION_DISABLE_NETWORK))
            disableNetwork(args, callbackContext);
        else if (action.equals(ACTION_DISCONNECT))
            disconnect(callbackContext);
        else if (action.equals(ACTION_ENABLE_NETWORK))
            enableNetwork(args, callbackContext);
        else if (action.equals(ACTION_GET_CONFIGURATION_NETWORKS))
            getConfiguredNetworks(callbackContext);
        else if (action.equals(ACTION_GET_CONNECTION_INFO))
            getConnectionInfo(callbackContext);
        else if (action.equals(ACTION_GET_DHCP_INFO))
            getDhcpInfo(callbackContext);
        else if (action.equals(ACTION_GET_SCAN_RESULTS))
            getScanResults(callbackContext);
        else if (action.equals(ACTION_GET_WIFI_AP_CONFIGURATION))
            return getWifiApConfiguration(callbackContext);
        else if (action.equals(ACTION_GET_WIFI_AP_STATE))
            return getWifiApState(callbackContext);
        else if (action.equals(ACTION_GET_WIFI_STATE))
            getWifiState(callbackContext);
        else if (action.equals(ACTION_IS_SCAN_ALWAYS_AVAILABLE))
            isScanAlwaysAvailable(callbackContext);
        else if (action.equals(ACTION_IS_WIFI_AP_ENABLED))
            return isWifiApEnabled(callbackContext);
        else if (action.equals(ACTION_IS_WIFI_ENABLED))
            isWifiEnabled(callbackContext);
        else if (action.equals(ACTION_REASSOCIATE))
            reassociate(callbackContext);
        else if (action.equals(SAVE_EAP_CONFIG))
            saveEapConfig(callbackContext, data);
        else if (action.equals(ACTION_RECONNECT))
            reconnect(callbackContext);
        else if (action.equals(ACTION_REMOVE_NETWORK))
            removeNetwork(args, callbackContext);
        else if (action.equals(ACTION_SAVE_CONFIGURATION))
            saveConfiguration(callbackContext);
        else if (action.equals(ACTION_SET_WIFI_AP_CONFIGURATION))
            return setWifiApConfiguration(args, callbackContext);
        else if (action.equals(ACTION_SET_WIFI_AP_ENABLED))
            return setWifiApEnabled(args, callbackContext);
        else if (action.equals(ACTION_SET_WIFI_ENABLED))
            setWifiEnabled(args, callbackContext);
        else if (action.equals(ACTION_START_SCAN))
            startScan(callbackContext);
        else if (action.equals(ACTION_UPDATE_NETWORK))
            updateNetwork(args, callbackContext);
        else if (action.equals(ACTION_ON_CHANGE))
    ...
    private boolean setWifiApEnabledWithPermission(JSONArray args, CallbackContext callbackContext)
            throws JSONException {
        Class<?> klass = wifiManager.getClass();
        JSONObject json = args.getJSONObject(0);
        WifiConfiguration wifiConfig = fromJSONWifiConfiguration(json);
    ...
    private static JSONObject toJSON(WifiConfiguration wifiConfig) throws JSONException {
        ...
        authAlgorithms.put("LEAP", wifiConfig.allowedAuthAlgorithms.get(WifiConfiguration.AuthAlgorithm.LEAP));
        authAlgorithms.put("OPEN", wifiConfig.allowedAuthAlgorithms.get(WifiConfiguration.AuthAlgorithm.OPEN));
        authAlgorithms.put("SHARED", wifiConfig.allowedAuthAlgorithms.get(WifiConfiguration.AuthAlgorithm.SHARED));
        json.put("allowedAuthAlgorithms", authAlgorithms);

        JSONObject groupCipher = new JSONObject();
        groupCipher.put("CCMP", wifiConfig.allowedGroupCiphers.get(WifiConfiguration.GroupCipher.CCMP));
        groupCipher.put("TKIP", wifiConfig.allowedGroupCiphers.get(WifiConfiguration.GroupCipher.TKIP));
        groupCipher.put("WEP104", wifiConfig.allowedGroupCiphers.get(WifiConfiguration.GroupCipher.WEP104));
        groupCipher.put("WEP40", wifiConfig.allowedGroupCiphers.get(WifiConfiguration.GroupCipher.WEP40));
        json.put("allowedGroupCiphers", groupCipher);

        JSONObject keyManagement = new JSONObject();
        keyManagement.put("IEEE8021X", wifiConfig.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.IEEE8021X));
        keyManagement.put("NONE", wifiConfig.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.NONE));
        keyManagement.put("WPA_EAP", wifiConfig.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.WPA_EAP));
        keyManagement.put("WPA_PSK", wifiConfig.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.WPA_PSK));
        json.put("allowedKeyManagement", keyManagement);

        JSONObject pairwiseCiphers = new JSONObject();
        pairwiseCiphers.put("CCMP", wifiConfig.allowedPairwiseCiphers.get(WifiConfiguration.PairwiseCipher.CCMP));
        pairwiseCiphers.put("NONE", wifiConfig.allowedPairwiseCiphers.get(WifiConfiguration.PairwiseCipher.NONE));
        pairwiseCiphers.put("TKIP", wifiConfig.allowedPairwiseCiphers.get(WifiConfiguration.PairwiseCipher.TKIP));
    ...
    private static WifiConfiguration fromJSONWifiConfiguration(JSONObject json) throws JSONException {
        ...
        if (!json.isNull("BSSID"))
            wifiConfig.BSSID = json.getString("BSSID");
        if (!json.isNull("SSID"))
            wifiConfig.SSID = json.getString("SSID");
        ...
        if (!json.isNull("hiddenSSID"))
            wifiConfig.hiddenSSID = json.getBoolean("hiddenSSID");
        if (!json.isNull("networkId"))
            wifiConfig.networkId = json.getInt("networkId");
        if (!json.isNull("preSharedKey"))
            wifiConfig.preSharedKey = json.getString("preSharedKey");
        if (!json.isNull("status"))
            wifiConfig.status = fromStringWifiConfigurationStatus(json.getString("status"));
        ...
        if (!json.isNull("wepKeys")) {
            ...
            for (int i = 0; i < wifiConfig.wepKeys.length; i++) {
                if (!wepKeys.isNull(i))
                    wifiConfig.wepKeys[i] = wepKeys.getString(i);
            }
        }

        if (!json.isNull("wepTxKeyIndex"))
            wifiConfig.wepTxKeyIndex = json.getInt("wepTxKeyIndex");
    ...
    private static String toStringWifiApState(int wifiApState) {
        if (wifiApState == WIFI_AP_STATE_DISABLED)
            return "DISABLED";
        else if (wifiApState == WIFI_AP_STATE_DISABLING)
            return "DISABLING";
        else if (wifiApState == WIFI_AP_STATE_ENABLED)
            return "ENABLED";
        else if (wifiApState == WIFI_AP_STATE_ENABLING)
            return "ENABLING";
        else if (wifiApState == WIFI_AP_STATE_FAILED)
            return "FAILED";
        else
            return null;
    }
    ...
    private boolean hasLocationPermission() {
        return cordova.hasPermission(ACCESS_COARSE_LOCATION) || cordova.hasPermission(ACCESS_FINE_LOCATION);
    }
            ...
            try {
                if (action.equals(WifiManager.NETWORK_IDS_CHANGED_ACTION)) {
                    ...
                    if (intent.hasExtra(WifiManager.EXTRA_SUPPLICANT_ERROR)) {
                        ...
                        String name = error == WifiManager.ERROR_AUTHENTICATING ? "ERROR_AUTHENTICATING"
                                : "ERROR_UNKNOWN";
    ...
    public WifiConfiguration setWifiConfigurations(WifiConfiguration selectedConfig, String mySSID, String userName,
            String userPass) {
        ...
        try {
            ...
            Field wcefAnonymousId = null, wcefCaCert = null, wcefClientCert = null, wcefEap = null, wcefIdentity = null,
                    wcefPassword = null, wcefPhase2 = null, wcefPrivateKey = null, wcefEngine = null,
                    wcefEngineId = null;
